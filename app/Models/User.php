...
class User extends Authenticatable implements Wallet
{
    ...
    private const SUB_AGENT_ROLE = 4;
    ...
    foreach ($this->customChildren as $child) {
        $ids = $ids->merge($child->getAllDescendants());
    }
    ...
    public function hasPermission($permission)
    {
        ...
        // Agent has permissions based on their assigned permissions
        ...
        // Player has limited permissions (check their assigned permissions)
    ...
    public function getAllDescendantPlayers()
    {
        ...
        $players = $this->children()->where('type', \App\Enums\UserType::Player->value)->get();

        // Fetch all sub-agents (agents under this agent)
        $subAgents = $this->children()->where('type', \App\Enums\UserType::Agent->value)->get();

        // For each sub-agent, fetch their players recursively (multi-level support)
        foreach ($subAgents as $subAgent) {
            $players = $players->merge($subAgent->getAllDescendantPlayers());
        }
