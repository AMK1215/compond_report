...
class PlayerController extends Controller
{
    ...
    public function player_with_agent()
    {
        ...
        $agents = User::with(['roles', 'players' => function($query) {
                $query->with('roles')
                    ->whereHas('roles', fn($q) => $q->where('role_id', self::PLAYER_ROLE))
                    ->orderBy('created_at', 'desc');
            }])
            ->whereHas('roles', fn($q) => $q->where('role_id', 2)) // Agent role
            ->whereHas('players', function($q) {
                $q->whereHas('roles', fn($q) => $q->where('role_id', self::PLAYER_ROLE));
            })
            ->orderBy('created_at', 'desc')
            ->get();
