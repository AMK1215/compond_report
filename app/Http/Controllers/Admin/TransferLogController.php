...
class TransferLogController extends Controller
{
    ...
    private function getDirectlyRelatedUserIds(User $user): array
    {
        ...
        if ($user->hasRole('Owner')) {
            // Owner: direct agents (all agents created by owner)
            $relatedIds = $user->children()->whereHas('roles', function ($q) {
                $q->where('title', 'Agent');
            })->pluck('id')->toArray();
        } elseif ($user->hasRole('Agent')) {
            // Agent: direct sub-agents, direct players, and parent (owner or parent agent)
            ...
            $parentId = $user->agent_id ? [$user->agent_id] : [];
            $relatedIds = array_merge($subAgentIds, $playerIds, $parentId);
        } elseif ($user->hasRole('Player')) {
            // Player: only parent agent
            $relatedIds = $user->agent_id ? [$user->agent_id] : [];
        }
