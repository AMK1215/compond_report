...
class BuffaloReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($user->type == UserType::Owner->value) {
        ...
        } elseif ($user->type == UserType::Agent->value) {
            // Agent can see their own players only (direct players under them)
            $playerIds = User::where('agent_id', $user->id)
        ...
        } elseif ($user->type == UserType::Player->value) {
        ...
        if ($user->type == UserType::Owner->value) {
            ...
            // Group by player for Agent/Player
        ...
        $agents = [];
        if ($user->type == UserType::Owner->value) {
            $agents = User::where('type', UserType::Agent->value)
                ->orderBy('user_name')
                ->get();
        }
        ...
        $players = [];
        if ($user->type == UserType::Owner->value) {
            $players = User::where('type', UserType::Player->value)->orderBy('user_name')->get();
        } elseif ($user->type == UserType::Agent->value) {
            $players = User::where('agent_id', $user->id)
                ->where('type', UserType::Player->value)
                ->get();
        }
    ...
    /**
     * Get report data grouped by player (for Agent/Player)
     */
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($type === 'agent') {
            ...
            if ($user->type == UserType::Agent->value) {
                $allowedPlayerIds = User::where('agent_id', $user->id)
                    ->where('type', UserType::Player->value)
                    ->pluck('id')
                    ->toArray();
