...
class BuffaloGameController extends Controller
{
    ...
    public function getUserBalance(Request $request)
    {
        ...
        // Verify token
        Log::info('6TriBet Buffalo - Token verification attempt', [
            'uid' => $uid,
            'token' => $token
        ]);
        
        if (!BuffaloGameService::verifyToken($uid, $token)) {
            Log::warning('6TriBet Buffalo - Token verification failed', [
                'uid' => $uid,
                'token' => $token
            ]);
        ...
        Log::info('6TriBet Buffalo - Token verification successful', [
            'uid' => $uid
        ]);
        ...
        if (!$userName) {
            Log::warning('6TriBet Buffalo - Could not extract username', [
                'uid' => $uid
            ]);
        ...
        Log::info('6TriBet Buffalo - Balance retrieved successfully', [
            'user' => $userName,
            'balance' => $balance
        ]);
