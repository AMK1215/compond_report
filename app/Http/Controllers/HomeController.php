...
class HomeController extends Controller
{
    ...
    {
        $user = Auth::user();
        $roleTitle = $user->roles->pluck('title')->first(); // 'Owner', 'Agent', 'Player'
        ...
        $totalBalance = DB::table('users')
            ...
            ->when(in_array($roleTitle, ['Owner', 'Agent']), function ($query) use ($user) {
        ...
        if (in_array($roleTitle, ['Owner', 'Agent'])) {
    ...
    {
        $totalOwner = $totalAgent = $totalPlayer = 0;
    ...
    private function getWinLose($id, $todayOnly = false): float
    {
        $query = DB::table('place_bets')
            ...
            ->where('place_bets.player_agent_id', $id);
