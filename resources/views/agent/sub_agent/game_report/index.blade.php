...
<div class="container mt-4">
    ...
    {{-- Filter Form --}}
    <form method="GET" action="{{ route('admin.player.game_report.index' ,$id) }}" class="row g-3 mb-3">
        <div class="col-md-3">
                ...
                <tbody>
                    @forelse($reports as $report)
                        <tr>
                            ...
                            <td>{{ number_format($report->prize_amount, 2) }}</td>
                            <td class="{{ $report->win_lose > 0 ? 'text-success' : 'text-danger' }}">{{ number_format($report->win_lose, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
